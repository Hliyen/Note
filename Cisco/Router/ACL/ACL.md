...
>TCP 具有「三次握手」與「確認回傳」機制。如果資料在傳輸途中掉了，TCP 會自動要求重傳。

    ● 特徵: 連線穩定、速度稍慢
    ● 常規搭配的Port:
        ● 網頁瀏覽（HTTP）：80 / （HTTPS）：443
        ● 遠端管理（SSH）：22 / （Telnet）：23
        ● 檔案傳輸（FTP）：21
        ● 郵件收發（SMTP）：25
    ● ACL 使用時機：要阻擋或放行某個具體的網頁、遠端連線或檔案服務時。
        指令範例：deny tcp any any eq 443（阻擋所有人看安全網頁）
### UDP ###
>「速度極快、即時、掉了就算了」

>UDP 屬於「射後不理」型。它只管把封包瘋狂往外丟，不管對方有沒有收到。

    ● 特徵: 速度極快、延遲極低
    ● 常規搭配的 Port：
        ● 網址解析（DNS）：53
        ● 自動動態分配 IP（DHCP）：67、68
        ● 影音串流 / 線上遊戲 / 語音通話 (VoIP)
    ● ACL 使用時機：要控制語法解析、即時影音、或者是像 OSPF（某些動態路由）/ 網卡自動取得 IP 的流量時。
        指令範例：permit udp any any eq 53 （放行所有人進行 DNS 解析）
### ICMP ###
>只想測試「網路通不通、設備活著沒」

>ICMP 不屬於 TCP 也不屬於 UDP，它是一個獨立的網際網路控制訊息協定。它完全沒有 Port（通訊埠）的概念。

    ● 特徵: 專門用來傳遞錯誤訊息和診斷網路
    ● 最著名的代表: ping 指令
    ● ACL 使用時機：想要「讓內網可以上網，但禁止外網的人 Ping 我們家的路由器」這種安全防護時。
        指令範例：deny icmp any any（禁止任何人 Ping 這台設備）
## ACL 運作的規則 ##
### 由上到下，依序匹配 ###
